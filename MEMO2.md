...
## 2026-09-22 · 第 32 次（本日第 4 次）：数据来源更正与 README 配图

### 【你】口径更正与说明
- 长时间考核实际是 60W，即 10V 6A，受上级电源与稳定性限制。
- 该照片是接近两小时时拍的，总目标 12 小时，仍在进行中。
- 功率板并非自研：参考 TI 评估板与嘉立创开源项目。
- 改板时修正了开源项目的已知问题，并借鉴其数控思路。
- 主控由原方案的 F407 换成本项目的 F103C8T6。

### 【我】本轮处理
- picture 目录纳入 8 张测试照片，README 加实拍小节用 6 张。
- 底板原图 8.7MB，另生成 334KB 的 web 版供 README 引用。
- 复核确认 UART 那处真错已修，调用处改传 DEV_UART2。
- 指标表拆成输出能力 120W 与长时间考核 60W 两行。
- 固件体积由 39.4KB 更正为 43.1KB，硬件来源描述一并改写。

### 【我】待你确认
- 简历那句"硬件自行设计焊接"的口径是否一并改。

## 待办（第 32 次）
- 推送前删掉 8.7MB 原图，只留 web 版，免得进历史。
- 简历硬件口径若要改，给一句我就替换。
- 长时间考核跑满 12 小时后回填最终结果。
- UART 水位加阈值动作，并把堆峰值一并上报。
- 循环开头置三个 state_res 为 SENSOR_SKIP，状态量初始化。
- 开栈溢出与 malloc 失败钩子，任务句柄逐个判空。
- 加 .gitattributes 与 renormalize，清掉行尾噪音。

## 2026-09-22 · 第 33 次（本日第 5 次）：硬件归属口径落进文档

### 【你】硬件归属补充口径
- 借鉴开源项目主要在布局，原理图依 TI 评估板与手册。
- 元件数值全部亲手计算，BOM 亲自选型。
- 数字控制思路大量借鉴开源项目。
- 芯片与结构不同：原方案一芯片一路 I2C，本设计合并。

### 【我】文档改动
- README 简介与硬件节改写，写明四层归属与两处结构差异。
- 硬件节新增与原方案的差异条目：主控降级与 I2C 合并。
- 简历背景改写为依 TI 评估板与手册设计，去掉自研口径。
- 简历职责由"软硬件独立开发"改为"软硬件开发"。
- 简历固件数字由 40KB 更正为 43KB，驱动条目点明合并总线。

### 【我】已闭合项
- 第 32 次待办里"简历硬件口径若要改"这条已完成。
